...
void MainWindow::mousePressEvent(QMouseEvent *event){
    if(event->button() == Qt::LeftButton){
        mousePressed = true;
        lastPos = event->globalPos().operator-=(this->frameGeometry().topLeft());
    }
    ...
    if(!mousePressed){
        ...
        if(mouseState == 0){
            if(maximized){
                ...
                controlWindowScale();
                this->move(QPoint(event->globalX() - ui->mainWidget->width() * wRatio, -30));
                lastPos = QPoint(ui->mainWidget->width() * wRatio, event->pos().y());
            }
            else
                this->move(event->globalPos().operator-=(lastPos));
        }
        else{
            QPoint d = event->globalPos().operator-=(frameGeometry().topLeft()+ lastPos);
