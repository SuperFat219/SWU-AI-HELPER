...
public class PKnightTour extends JApplet {
    ...
    public class Square implements Comparable<Square> {
        ...
        public java.util.ArrayList<Square> getPossibleMoves(boolean includeVisited){
            ...
            if (dim * getRow() >= id + 1) {
                //can move right 1
                if (id + 1 + dim * 2 <= dim * dim){
                    square = board.getSquare(id + 1 + dim * 2);
                    if (!square.getVisited() || includeVisited)
                        moves.add(square);
                }
                    
                if ((id + 1 - dim * 2 <= dim * dim) && (id + 1 - dim * 2 > 0)) {
                    square = board.getSquare(id + 1 - dim * 2);
                    if (!square.getVisited() || includeVisited)
                        moves.add(square);
                }
    ...
    private class SquareStack extends java.util.Stack<Square> {
        
        public static final long serialVersionUID = 1L;
                
        public Square push(Square item){
        ...
        public SquareStack reverse(){
            SquareStack newStack = new SquareStack();
    ...
    private static class ChessboardJFrame extends javax.swing.JFrame {
        ...
        private void plotSolution(Integer[] path){
            ...
            for (int i = 0; i < path.length; i++){
                ...
                for(int c = 0; c < comps.length; c++){
                    ...
                    if ((comps[c].getName() != null) && 
                            (path[i].compareTo(Integer.parseInt(comps[c].getName())) == 0)) {
                        ((JLabel)comps[c]).setText(Integer.toString((i + 1)));
                    }
        ...
        private void squareClick(java.awt.event.MouseEvent evt) {                             
           if (!touring){
               touring = true;
                ...
                PKnightTour kt = new PKnightTour(Integer.parseInt(l.getName()));

                Integer[] path = kt.getSolution();
                plotSolution(path);
    ...
    //            public void run() {
    //                new KnightsTourJFrame().setVisible(true);
    //            }
    //        });
    //    }
